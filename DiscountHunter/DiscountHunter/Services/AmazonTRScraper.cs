...
public class AmazonTRScraper : BaseScraper
{
    public override string StoreName => "Amazon TR";

    public AmazonTRScraper(decimal minDiscountPercent = 20) : base(minDiscountPercent) { }
    ...
    public override async Task<ScrapeResult> ScrapeByKeywordAsync(string keyword)
    {
        ...
        try
        {
            ...
            var searchUrl = $"https://www.amazon.com.tr/s?k={Uri.EscapeDataString(keyword)}";
    ...
    public override async Task<ScrapeResult> ScrapeByCategoryAsync(string categoryUrl)
    {
        ...
        try
        {
            ...
            if (!categoryUrl.StartsWith("http"))
                categoryUrl = $"https://www.amazon.com.tr{categoryUrl}";
    ...
    private async Task<List<Product>> ExtractProductsAsync()
    {
        ...
        try
        {
            var productCards = await _page!.QuerySelectorAllAsync("[data-component-type='s-search-result']");
            ...
            foreach (var card in productCards.Take(50))
            {
                try
                {
                    var nameEl = await card.QuerySelectorAsync("h2 a span, .a-size-medium");
                    var priceWholeEl = await card.QuerySelectorAsync(".a-price .a-offscreen, .a-price-whole");
                    var originalPriceEl = await card.QuerySelectorAsync(".a-text-price .a-offscreen, span[data-a-strike='true']");
                    var linkEl = await card.QuerySelectorAsync("h2 a, a.a-link-normal");

                    if (nameEl == null || priceWholeEl == null) continue;

                    var name = await nameEl.InnerTextAsync();
                    var discountedPrice = ParsePrice(await priceWholeEl.InnerTextAsync());
                    ...
                    if (linkEl != null)
                    {
                        ...
                        url = href?.StartsWith("http") == true ? href : $"https://www.amazon.com.tr{href}";
                    }
    ...
    private async Task<Product?> ExtractSingleProductAsync(string url)
    {
        try
        {
            var nameEl = await _page!.QuerySelectorAsync("#productTitle");
            var priceEl = await _page.QuerySelectorAsync(".a-price .a-offscreen, #priceblock_ourprice, #priceblock_dealprice");
            var originalPriceEl = await _page.QuerySelectorAsync(".a-text-price .a-offscreen, #priceblock_ourprice_lbl + span");
